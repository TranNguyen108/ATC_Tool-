...
async def find_comment_form(page: Page) -> FormResult:
    ...
    if form is None:
        candidates = [
            page.locator("form[id*='comment' i]"),
            page.locator("form[class*='comment' i]"),
            page.locator("form[id*='respond' i]"),
            page.locator("#respond form"),
            page.locator(".comment-respond form"),
            page.locator(".comments-area form"),
            page.locator("form[action*='comment' i]"),
            page.locator("form[class*='review' i]"),
            page.locator("form[id*='review' i]"),
            page.locator(".review-form form"),
            page.locator("#review_form"),
            page.locator("#review_form_wrapper form"),
            page.locator(".woocommerce-Reviews form"),
        ]
        for cand in candidates:
            if await _is_visible(cand):
                form = cand.first
                result.strategy = "id_class_comment"
                break
        # Nếu không visible, check DOM rồi thử reveal
        if form is None:
            for cand in candidates:
                if await _exists_in_dom(cand):
                    try:
                        await cand.first.evaluate("el => { el.style.display = ''; el.style.visibility = 'visible'; }")
                    except Exception:
                        pass
                    if await _is_visible(cand):
                        form = cand.first
                        result.strategy = "id_class_comment_revealed"
    ...
    if form is None:
        ...
        for c_sel in container_sels:
            ...
            if await _is_visible(c_loc):
                if await c_loc.locator("textarea").count() > 0:
                    form = c_loc.first
                    result.strategy = "container_no_form"
                    log.debug(f"Layer 5: found textarea in container {c_sel}")
                    break

...
class JobScheduler:
    ...
    def next_job(self) -> tuple[str, str, str] | None:
        """Tra ve (url, comment, key) hoac None neu het viec.
        Duyet qua tat ca key (khong chi key hien tai) de tim job kha thi.
        Tra ve None chi khi THUC SU khong con gi de lam.
        """
        with self._lock:
            if not self._urls:
                return None  # Het URL hoan toan

            # Duyet tat ca key tu _key_idx den het, tim key co pool va chua du target
            tried = 0
            ...
            while tried < total_keys:
                ...
                if not pool:
                    # Het comment tam thoi (dang in-flight) — thu key khac
                    tried += 1
                    continue

                # Co ca URL lan comment — dispatch job
                url = self._urls.popleft()
                comment = pool.popleft()
                self._inflight[url] = (comment, key)
                # Round-robin: nhay sang key TIEP THEO de lan goi sau
                # khong dispatch cho cung 1 key lien tuc (tranh starvation)
                self._key_idx = (idx + 1) % total_keys
                return (url, comment, key)

            return None  # Tat ca key hoac da du, hoac tam thoi het comment
    ...
    def is_done(self) -> bool:
        with self._lock:
            # Neu con URL dang in-flight thi chua xong (comment co the tra ve pool)
            if self._inflight:
                return False
            # Kiem tra tat ca key
            ...
            for key, target in self._keys:
                if self._success[key] < target:
                    ...
                    # Key nay can them — co tai nguyen de lam tiep khong?
                    if self._urls and self._pools.get(key):
                        return False  # Con URL va comment → con viec de lam
            # Neu tat ca key da du target → done
            if all_targets_met:
                return True
            # Chua du target nhung het tai nguyen (URL hoac comment) → done
            return True
    ...
    def pending_count(self) -> int:
        ...
        with self._lock:
            return len(self._urls) + len(self._inflight)
...
# ─────────────────────────────────────────────
# Block resources nặng → load nhanh gấp 3-5x
# ─────────────────────────────────────────────

# KHÔNG block stylesheet — nếu block CSS thì element display:none (search form,
# newsletter form) trở thành visible → form_detect tìm NHẦM form → treo.
_BLOCKED_TYPES = {"image", "media", "font"}

# Precompile 1 lan — nhanh hon any() tuyen tinh O(n)
_BLOCKED_URL_RE = re.compile(
    r"google-analytics\.com|googletagmanager\.com"
    r"|facebook\.net|doubleclick\.net"
    r"|adservice|analytics|tracker"
    r"|\.woff2?|\.ttf|\.otf"
...
def _should_block(route) -> bool:  # sync — khong can async
    """Kiem tra request co nen block khong."""
    req = route.request
    if req.resource_type in _BLOCKED_TYPES:
        return True
    return bool(_BLOCKED_URL_RE.search(req.url))
...
async def _process_url(
    ...
    try:
        ...
        # ── Block resources nang (images, CSS, fonts, trackers) ──
        async def _route_handler(route):
            # Boc trong try/except: context co the da dong khi route fire
            # → tranh "Future exception was never retrieved"
            try:
                ...
                pass  # context/page da dong — bo qua
        ...
        # ── Auto anchor text: thay key → <a href="host">key</a> ──
        # Chi replace khi comment CHUA co the <a> nao bao key
        # Tranh double-wrap: <a href="https://<a href="...">key</a>...">key</a>
        if profile.host and key and key in comment:
            already_has_anchor = bool(re.search(
                r'<a\s[^>]*>' + re.escape(key) + r'</a>',
                comment, re.IGNORECASE,
            ))
            if not already_has_anchor:
                anchor = f'<a href="{profile.host}">{key}</a>'
                comment = comment.replace(key, anchor, 1)
                log.debug(f"[{key}] Anchor converted: {anchor}")
            else:
                log.debug(f"[{key}] Comment da co anchor — giu nguyen")
    ...
    async with async_playwright() as pw:
        browser: Browser = await pw.chromium.launch(
            ...
            args=[
                ...
                # ── Extra memory flags ──
                # NOTE: --single-process removed — gây crash TargetClosedError dưới memory pressure
        ...
        try:
            # ── Vong lap ngoai: restart sau watchdog ─────────────────────
            MAX_OUTER_RESTARTS = 50  # Gioi han restart de tranh loop vo tan
            outer_restarts = 0
            _urls_since_restart = 0   # Dem URL de periodic restart giai phong RAM
            _RESTART_EVERY = 200      # Restart browser contexts moi 200 URL
            while not stop_event.is_set():
                if scheduler.is_done():
                    log.info(f"[SESSION] scheduler.is_done()=True — ket thuc. "
                             f"Status: {scheduler.status_summary()}")
                    break

                # Browser crash → dừng ngay, không tạo worker rồi fail ngay lập tức
                if not browser.is_connected():
                    log.error("[SESSION] Browser process died before worker start — stopping")
                    result_queue.put(Result(
                        status=STATUS_WATCHDOG,
                        error_detail="Browser process crash — session stopped",
                    ))
                    break
                ...
                async def worker_loop(wid: int,
                                      _lrt=last_result_time,
                                      _wt=watchdog_triggered) -> None:
                    """Moi worker loop: lay job -> xu ly -> bao cao -> lap lai.
                    Khi next_job() tra None tam thoi (comment dang in-flight),
                    doi 3s roi thu lai — chi dung khi scheduler.is_done() thuc su.
                    """
                    empty_retries = 0
                    MAX_EMPTY_RETRIES = 25  # 25 × 3s = 75s > _URL_TIMEOUT (60s)
                    ...
                    while not stop_event.is_set() and not _wt.is_set():
                        ...
                        if job is None:
                            if scheduler.is_done():
                                break  # Thuc su xong
                            # Tam thoi het — doi comment tra ve tu in-flight
                            empty_retries += 1
                            if empty_retries > MAX_EMPTY_RETRIES:
                                log.info(f"[W{wid}] Cho qua {MAX_EMPTY_RETRIES} lan, dung")
                                break
                            log.debug(f"[W{wid}] next_job() None — cho {empty_retries}/{MAX_EMPTY_RETRIES}")
                            await asyncio.sleep(3)
                            continue

                        empty_retries = 0  # Reset khi co job moi
                        url, comment, key = job
                        ...
                        try:
                            final_status = await asyncio.wait_for(
                                _process_url(
                                    url, comment, key,
                                    profile, browser, result_queue, loop, headless,
                            ...
                            # Watchdog cancel — khong finalize, recover_inflight se xu ly
                            raise
                        except asyncio.TimeoutError:
                            log.error(f"[W{wid}][{key}] URL TIMEOUT ({_URL_TIMEOUT}s) {url}")
                            # Force-close context bi treo — nguyen nhan chinh leak RAM
                            await _force_close_context(wid, "timeout")
                            result_queue.put(Result(
                                url=url, status=STATUS_REVIEW, key=key,
                                comment_used=comment, comment_link=url,
                                error_detail=f"URL xu ly qua {_URL_TIMEOUT}s — force skip",
                            ))
                            ...
                            _active_contexts.pop(wid, None)
                            scheduler.return_comment(key, comment)
                            scheduler.finalize_job(url)
                            _lrt[0] = time.monotonic()
                            result_queue.put(Result(status=STATUS_PROGRESS, key=key))
                            # Browser process chết → dừng toàn session, không retry vô ích
                            if not browser.is_connected():
                                log.error(f"[W{wid}] Browser process died — stop_event set")
                                result_queue.put(Result(
                                    status=STATUS_WATCHDOG,
                                    error_detail="Browser process crash — session stopped",
                                ))
                            ...
                            log.error(f"[W{wid}][{key}] UNHANDLED {url} — {exc}")
                            # Force-close possible zombie
                            await _force_close_context(wid, "unhandled")
                            result_queue.put(Result(
                                url=url, status=STATUS_REVIEW, key=key,
                                comment_used=comment, comment_link=url,
                                error_detail=f"worker_loop unhandled: {exc}",
                            ))
                            scheduler.return_comment(key, comment)
                            scheduler.finalize_job(url)
                            _lrt[0] = time.monotonic()
                        ...
                        # Periodic restart de giai phong Chromium renderer memory
                        nonlocal _urls_since_restart
                        _urls_since_restart += 1
                        if _urls_since_restart >= _RESTART_EVERY:
                            log.info(f"[W{wid}] Da xu ly {_RESTART_EVERY} URLs — trigger restart de giai phong RAM")
                            _wt.set()
                            break
                    log.info(f"[W{wid}] worker finished")

                # ── Watchdog coroutine ──────────────────────────────────
                async def watchdog_loop(_lrt=last_result_time,
                                        _wt=watchdog_triggered) -> None:
                    while not stop_event.is_set() and not _wt.is_set():
                        await asyncio.sleep(WATCHDOG_CHECK)
                        if stop_event.is_set() or _wt.is_set():
                            break
                        elapsed = time.monotonic() - _lrt[0]
                        if elapsed >= WATCHDOG_TIMEOUT:
                            log.warning(
                                f"[WD] Khong co ket qua trong {elapsed:.0f}s "
                                f"— kich hoat restart workers"
                            )
                            _wt.set()
                            break
                ...
                # Hard cap 8h cho toan session (du cho 88K+ URLs)
                try:
                    await asyncio.wait_for(
                    ...
                    log.error("SESSION hard timeout 8h — force stopping all workers")
                    stop_event.set()
                    for t in tasks:
                        if not t.done():
                            t.cancel()
                ...
                # Cancel watchdog neu worker xong truoc
                if not wd_task.done():
                    wd_task.cancel()
                try:
                    await wd_task
                ...
                if watchdog_triggered.is_set() and not stop_event.is_set():
                    ...
                    # Danh dau URL chet la FAILED de GUI hien thi
                    for dead_url, dead_cmt, dead_key in dead:
                        result_queue.put(Result(
                            url=dead_url,
                            status=STATUS_REVIEW,
                            key=dead_key,
                            comment_used=dead_cmt,
                            comment_link=dead_url,
                            error_detail=f"Watchdog: URL treo qua {_MAX_URL_RECOVERIES} lan — bo qua",
                        ))
                        result_queue.put(Result(status=STATUS_PROGRESS, key=dead_key))

                    msg = (f"Watchdog: phuc hoi {recovered} URL"
                           + (f", bo qua {len(dead)} URL bi hong" if dead else "")
                           + ", dang khoi dong lai...")
                    log.warning(f"[WD] {msg}")
                    result_queue.put(Result(status=STATUS_WATCHDOG, error_detail=msg))

                    # ── Close ALL zombie contexts — tránh memory leak ──
                    try:
                        contexts_to_close = list(browser.contexts)
                        ...
                        log.info(f"[WD] Force-closed {len(contexts_to_close)} zombie contexts")
                    except Exception:
                        pass

                    _urls_since_restart = 0  # Reset counter sau restart
                    await asyncio.sleep(3)  # nghi truoc khi restart
                    outer_restarts += 1
                    if outer_restarts >= MAX_OUTER_RESTARTS:
                        log.error(f"[SESSION] Qua {MAX_OUTER_RESTARTS} lan restart — force stop")
                        break
                    continue   # quay lai vong lap ngoai → tao worker moi

                # Workers xong — kiem tra scheduler that su done chua
                if not scheduler.is_done():
                    # Workers dung nhung chua xong — co the do MAX_EMPTY_RETRIES
                    # hoac TargetClosedError. Khoi dong lai workers.
                    log.warning(
                        f"[SESSION] Workers dung truoc khi scheduler done. "
                        f"Status: {scheduler.status_summary()}. Khoi dong lai..."
                    )
                    result_queue.put(Result(
                        status=STATUS_WATCHDOG,
                        error_detail="Workers dung som — khoi dong lai...",
                    ))

                    # Close ALL context zombie
                    try:
                        contexts_to_close = list(browser.contexts)
                        for ctx in contexts_to_close:
                            try:
                ...
                break  # scheduler.is_done() == True — ket thuc that su

        finally:
            try:
                await asyncio.wait_for(browser.close(), timeout=10)
            except Exception:
                log.warning("browser.close() timeout — force skip")
